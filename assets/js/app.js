...
const app = {
            ...
            if (data.code == 200) {
                                ...
                                <div class="info-song">
                                    <div class="info-song-wrap">
                                        ...
                                        <div class="col3">
                                            ...
                                            <div class="topicSong" idTopic=${
                                                song.idTopic
                                            }># ${song.topicTitle}</div>
                                            <div class="${
                                                song.statusSecurity == "public"
                                                    ? "bg-success"
                                                    : "bg-danger"
                    ...
                    btnPlays.forEach((btn) => {
                        btn.addEventListener("click", () => {
                            ...
                            if (songNode) {
                                ...
                                const image =
                                    songNode.querySelector(".image").src;
                                const audio =
                                    songNode.querySelector(".audio").src;
                ...
                btnEditSongs.forEach((btn) => {
                    btn.addEventListener("click", () => {
                        ...
                        let songEdit = listSong.find(
                            (song) =>
                                song._id == songNodeEdit.getAttribute("idSong")
                        );

                        const inputTitle = editSongContainer.querySelector(
                            'input[name="title"]'
                        );
                        const inputTopic = editSongContainer.querySelector(
                            'input[name="topic"]'
                        );
                        const inputDescription =
                            editSongContainer.querySelector(
                                'input[name="description"]'
                            );
                        const inputImagePreview =
                            editSongContainer.querySelector(".rounded-circle");
                        const inputUploadAvatar =
                            editSongContainer.querySelector(
                                ".upload-image-input"
                            );
                        const btnSaveEdit =
                            editSongContainer.querySelector(".button-save");
                        ...
                        async function setFileFromUrl(inputElement, imageUrl) {
                            try {
                                ...
                                console.log(
                                    "Hình ảnh đã được gán vào input file!",
                                    inputElement.files
                                );
                            } catch (error) {
                        ...
                        btnSaveEdit.addEventListener("click", async () => {
                            ...
                            const statusSecurity =
                                editSongContainer.querySelector(
                                    'input[name="statusSecurity"]:checked'
                                ).value;
                            ...
                            try {
                                ...
                                const response = await fetch(
                                    "https://server-musiczone.vercel.app/api/v1/songs/edit/" +
                                        idSong,
                                    {
                                        method: "PATCH",
                                        body: formData,
    ...
    handleEditProfile: function () {
        ...
        const inputUserName = formEditProfile.querySelector(
            'input[name="userName"]'
        );
        const inputCompany = formEditProfile.querySelector(
            'input[name="company"]'
        );
        const inputAddress = formEditProfile.querySelector(
            'input[name="address"]'
        );
        const inputIntroduce = formEditProfile.querySelector(
            'input[name="introduce"]'
        );
        const btnSave = formEditProfile.querySelector(".button-save");
        const imageAvatarPreview =
            formEditProfile.querySelector(".rounded-circle");
        const inputAvatar = formEditProfile.querySelector(
            ".upload-image-input"
        );
        ...
        btnSave.addEventListener("click", async () => {
            ...
            try {
                // Gửi request lên API
                const response = await fetch(
                    "https://server-musiczone.vercel.app/api/v1/user/edit",
                    {
                        method: "PATCH",
                        headers: {
                            Authorization: `Bearer ${app.getCookie("token")}`,
